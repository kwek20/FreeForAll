...
public class InventoryEngine extends Engine {
    ...
    public void onPlayerEnteredArena(PlayerEnteredArenaEvent evt) {
        if (!evt.isCancelled()) {
            ...
            if (!evtPlayer.isDead() || evt.getMethod() == MoveMethod.RESPAWNED) {
                ...
                evtPlayer.sendMessage(ChatColor.YELLOW + "[FreeForAll]" + ChatColor.RED + " You have entered the arena. Your inventory has been saved.");
    ...
    public boolean restoreInventory(Player player) {
        ...
        if (pStorage != null) {
            ...
            SE.removeStorage(pStorage);
            player.sendMessage(ChatColor.YELLOW + "[FreeForAll]" + ChatColor.RED + " Your inventory has been restored.");
            return true;
        }
        return false;
    }
